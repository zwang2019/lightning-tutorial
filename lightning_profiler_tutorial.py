...
if __name__ == '__main__':
    ...
    # Logger
    logger = TensorBoardLogger(save_dir='./tensorboardlogger', name='tutorial_model_v1', version=1)

    # Profiler
    profiler = PyTorchProfiler(
        on_trace_ready=torch.profiler.tensorboard_trace_handler('./tensorboardlogger/profiler0'),
        trace_memory=True,
        schedule=torch.profiler.schedule(skip_first=10, wait=1, warmup=1, active=20)
    )

    trainer = L.Trainer(profiler=profiler, logger=logger, accelerator='gpu', devices=1, strategy='auto', max_epochs=CONFIG.num_epochs, callbacks=[checkpoint_callback, early_stopping, timer])
